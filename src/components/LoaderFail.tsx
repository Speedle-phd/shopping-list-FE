...
const LoaderFail = () => {
  ...
  return (
    <Flex justifyContent={"center"} alignItems={"center"} height="500px" flexDirection={"column"}>
      ...
      <Text marginBlockStart={5} fontWeight={600} fontSize={"1.1rem"} color="rgba(0,0,0,.5)">{loaderData?.statuscode ?? routeError?.statuscode} - {loaderData?.msg ?? routeError?.msg}</Text>
